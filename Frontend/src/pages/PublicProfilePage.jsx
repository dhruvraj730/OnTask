        ...
        <div className="min-h-screen bg-gray-50 pb-20 pt-10 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-4 mb-2">
                                ...
                                <div className="flex items-center gap-1 bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-sm font-bold">
                                    ...
                                    {profile.rating || 'N/A'}
                                    <span className="text-gray-400 font-normal ml-1">{profile.jobsCompleted || 0} jobs</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-extrabold text-gray-900">{profile.jobsCompleted || 0}</p>
                                </div>
                                <div>
                                    <p className="text-xs font-bold text-gray-400 uppercase mb-1">Total Earnings</p>
                                    <p className="text-2xl font-extrabold text-green-600">₹{profile.totalEarnings?.toLocaleString() || 0}</p>
                                </div>
                                <div>
                    ...
                    {activeTab === 'Portfolio' && (
                        <div className="space-y-4">
                            {profile.portfolio && profile.portfolio.length > 0 ? profile.portfolio.map((item, idx) => (
                                <GlassContainer key={idx} className="p-6 border border-gray-100 shadow-lg hover:shadow-xl transition-all cursor-pointer group">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-1 font-bold text-orange-500">
                                            <Star className="w-5 h-5 fill-current" /> {item.rating}
                                        </div>
                                    </div>
                                </GlassContainer>
                            )) : (
                                <p className="text-center py-10 text-gray-500">No portfolio items added yet.</p>
                            )}
                        </div>
                    )}
                ...
                <div className="flex gap-4 max-w-2xl mx-auto">
                    ...
                    <button className="flex-[2] py-4 px-8 bg-green-600 text-white font-extrabold rounded-2xl hover:bg-green-700 transition-all shadow-xl shadow-green-600/20">
                        Hire Now
                    </button>
                </div>
            </div>
